...
static int sendPacket(MQTTClient* c, int length, Timer* timer)
{
    int rc = MQTT_FAILURE,
        sent = 0;
#if 1
    // krl: kludge to fix case where timer expires just when we are about to send a (reply) packet
    Timer tmp;
    if(TimerLeftMS(timer) < 10) {
    	//printf("write kludge\r\n");
		TimerInit(&tmp);
		TimerCountdownMS(&tmp, 10);
		timer = &tmp;
    }
    // krl: end of kludge
#endif
       ...
       messageHandler messageHandler, MQTTSubackData* data)
{
    ...
    len = MQTTSerialize_subscribe(c->buf, c->buf_size, 0, getNextPacketId(c), 1, &topic, &intQos);
    if (len <= 0)
        goto exit;
    ...
    if (waitfor(c, SUBACK, &timer) == SUBACK)      // wait for suback
    {
        ...
        intQos = data->grantedQoS;
        if (MQTTDeserialize_suback(&mypacketid, 1, &count, &intQos, c->readbuf, c->readbuf_size) == 1)
        {
        	data->grantedQoS = intQos;
