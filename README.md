...
- #### Interfaces
    ...
    2. ###### LCD Interface: 
		 ...
		 A word on the interface flags. First flag to appear after the sceen name is **!** - it marks if the desired pressure level cannot be achieved by the fan in automatic mode. Second flag is **\*** - it appears on the sceen when device will not respond to any commands, while it performs some task. Third flag is **U** and **D** - meaning that current selected sensor or actuator corresponding to the sceen is up or down accordingly. Fourth flag is **M** and **A** - meaning the device operates in manual or automatic mode.
